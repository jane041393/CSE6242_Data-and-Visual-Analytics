...
val file = spark.read.textFile("/FileStore/tables/toygraph.csv") 
...
val high_out = filter.groupBy("Source").count().withColumnRenamed("count", "out_score").sort($"out_score".desc)
val high_in = filter.groupBy("Target").count().withColumnRenamed("count", "in_score").sort($"in_score".desc)

val combination = high_out.join(high_in, high_out("Source") === high_in("Target"),"fullouter").withColumn("Source",when($"Source".isNull,$"Target").otherwise($"Source")).sort($"Source".asc).na.fill(0).select($"Source",$"out_score",$"in_score").withColumn("Total_score", $"out_score" + $"in_score")

val high_total = combination.select($"Source",$"Total_score").sort($"Total_score".desc).first()
val high_out1 = high_out.first()
val high_in1 = high_in.first()


// COMMAND ----------


val result = Seq((high_in1.getString(0),high_in1.getLong(1),"i"), 
                  (high_out1.getString(0),high_out1.getLong(1),"o"),
                  (high_total.getString(0),high_total.getLong(1),"t")).toDF("v","d","c")
